(function($) {
  ...
  var wcdAnimControl = function(sel){
    ...
    this.spriteData = function(sel) {
      var thisSprite = document.querySelector('[data-sprite="' + sel + '"]');
      //specific selector for ebb
      var thisContainer = document.querySelector('.linen-ebb');
      that.sprite = {
        el :  thisSprite,
    ...
    this.constructSprite = function(item){
      ...
      that.loadSprite(that.sprite, function(e){
        ...
        if($sprite.playback == "hover"){
          ...
          $sprite.container.addEventListener('mouseenter', function(){ 
            if(that.paused)that.play();
          });

          $sprite.container.addEventListener('mouseleave', function(){ 
            if(!that.paused)that.pause();
          });
        }else {
